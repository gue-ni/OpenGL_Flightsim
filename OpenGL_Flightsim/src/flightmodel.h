...
float get_propellor_thrust(const phi::RigidBody& rb, float engine_horsepower,
                           float propellor_rpm, float propellor_diameter) {
  float speed = rb.get_speed();
  float engine_power = phi::units::watts(engine_horsepower);

#if 1
  const float a = 1.83f, b = -1.32f;
  float propellor_advance_ratio =
      speed / ((propellor_rpm / 60.0f) * propellor_diameter);
  float propellor_efficiency =
      a * propellor_advance_ratio + std::pow(b * propellor_advance_ratio, 3.0f);
#else
  ...
  float power_drop_off_factor =
      ((air_density / sea_level_air_density) - C) / (1 - C);
#else
  float power_drop_off_factor = 1.0f;
#endif

  return ((propellor_efficiency * engine_power) / speed) *
         power_drop_off_factor;
}
...
float get_indicated_air_speed(const phi::RigidBody& rb) {
  ...
  const float dynamic_pressure =
      0.5f * air_density * phi::sq(airspeed);  // bernoulli's equation
...
float get_g_force(const phi::RigidBody& rb) {
  ...
  g_force += (rb.up().y * phi::EARTH_GRAVITY) /
             phi::EARTH_GRAVITY;  // add earth gravity
  return g_force;
}
...
struct Airfoil {
  ...
  std::tuple<float, float> sample(float alpha) const {
    ...
    int index = static_cast<int>(phi::utils::scale(
        alpha, min_alpha, max_alpha, 0.0f, static_cast<float>(max_index)));
...
struct Wing : public phi::ForceEffector {
  ...
  Wing(const glm::vec3& position, float area, const Airfoil* aero,
  ...
  Wing(const glm::vec3& position, float wingspan, float chord,
       const Airfoil* aero, const glm::vec3& normal = phi::UP)
      : center_of_pressure(position),
        area(chord * wingspan),
        airfoil(aero),
        normal(normal) {}
  ...
  void apply_forces(phi::RigidBody& rigid_body) override {
    ...
    if (std::abs(deflection) > phi::EPSILON) {
      ...
      auto rotation =
          glm::rotate(glm::mat4(1.0f), glm::radians(deflection), axis);
    ...
    glm::vec3 lift_direction = glm::normalize(
        glm::cross(glm::cross(drag_direction, wing_normal), drag_direction));

    // angle between chord line and air flow
    float angle_of_attack =
        glm::degrees(std::asin(glm::dot(drag_direction, wing_normal)));

    // sample our aerodynamic data
    auto [lift_coefficient, drag_coefficient] =
        airfoil->sample(angle_of_attack);
...
struct Aircraft {
  ...
  Aircraft(float mass, float thrust, glm::mat3 inertia, std::vector<Wing> wings)
      : elements(wings),
        rigid_body({.mass = mass, .inertia = inertia}),
        engine(thrust) {}
